...
int main(void)
{
	...
	for ( count = 0; count < MAX ; count++ )
	{
		...
		std::cin >> tab[count];//std::cin >> tab[1];
	}
	for ( count = 0; count < MAX ; count++ )//for ( count = 1; count < MAX ; count++ )
...
int max(int min[], int max)
{
	int biggest;
	biggest = 0;
	for (int i = 0; i < max; i++)//for (int i = 1; i < max; i++)
		if (biggest <= min[i])
			biggest = min[i];//biggest = min[0];
	return biggest;
}
